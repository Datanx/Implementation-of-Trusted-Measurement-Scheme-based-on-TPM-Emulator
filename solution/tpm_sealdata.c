#include <openssl/evp.h>        //通用加密算法接口
#include <limits.h>             //规定数据类型范围，增强代码可移植性
#include "tpm_tspi.h"
#include "tpm_utils.h"
#include "tpm_seal.h"
...
static int parse(const int aOpt, const char *aArg)             //解析函数
{
	int rc = -1;
...
int main(int argc, char **argv)
{
	...
	TSS_HENCDATA hEncdata;        //加密数据对象句柄
	...
	UINT32 sealKeyLen;
	BYTE *sealKey;
	TSS_FLAG keyFlags = TSS_KEY_TYPE_STORAGE | TSS_KEY_SIZE_2048 |        //存储子秘钥属性：存储秘钥；2048位；秘钥是易变的；必须在启动时卸载；
	    TSS_KEY_VOLATILE | TSS_KEY_AUTHORIZATION |                        //秘钥需要授权；秘钥不可迁移
	...
	if (genericOptHandler(argc, argv, "i:o:p:uz", opts,                   //通用选项处理函数
	...
	if ((bin = BIO_new(BIO_s_file())) == NULL) {                       //封装了标准的文件接口的BIO，包括标准输入输出设备如stdin
	...
	if ((b64 = BIO_new(BIO_f_base64())) == NULL) {                    //封装了base64编码方法的BIO,写的时候进行编码，读的时候解码
